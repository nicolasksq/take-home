...
func (mc Mailchimp) GetListsByName(name string) (*dao.List, error) {
	...
	getParams := lists.GetParams{
		Fields: []string{"lists.id", "lists.name", "total_items"},
		Count:  count,
		Offset: offset,
	}

	var total int
	var response *lists.Lists
	var err error

	for total >= getParams.Offset {
		response, err = lists.Get(&getParams)
		if err != nil {
			return nil, err
		}
...
func (mc Mailchimp) BatchListMembers(contacts []dao.Contact, listID string) ([]dao.Contact, error) {
	...
	membersResponse, err := members.New(listID, params)
	if err != nil {
		return nil, err
	}
